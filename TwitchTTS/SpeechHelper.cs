...
namespace TwitchTTS
{
	public static class SpeechApiReflectionHelper
	{
		...
		private static readonly Type ObjectTokenType = typeof(SpeechSynthesizer).Assembly
			.GetType("System.Speech.Internal.ObjectTokens.ObjectToken")!;

		private static readonly Type ObjectTokensListType = typeof(IList<>).MakeGenericType(new Type[]{ObjectTokenType});
		...
		{
			var voiceSynthesizer = GetProperty(synthesizer, PROP_VOICE_SYNTHESIZER) ?? throw new NotSupportedException($"Property not found: {PROP_VOICE_SYNTHESIZER}");
			var installedVoices = GetField(voiceSynthesizer, FIELD_INSTALLED_VOICES) as List<InstalledVoice> ?? throw new NotSupportedException($"Field not found or null: {FIELD_INSTALLED_VOICES}");
						...
						var voiceInfo =
							typeof(SpeechSynthesizer).Assembly
								.CreateInstance(VoiceInfoType.FullName!, true,
									...
									new object[] {token}, null, null) ?? throw new NotSupportedException($"Failed to instantiate {VoiceInfoType}");
						var installedVoice =
							typeof(SpeechSynthesizer).Assembly
								.CreateInstance(InstalledVoiceType.FullName!, true,
									BindingFlags.Instance | BindingFlags.NonPublic, null,
									new object[] {voiceSynthesizer, voiceInfo}, null, null) as InstalledVoice ?? throw new NotSupportedException($"Failed to instantiate {InstalledVoiceType}");
